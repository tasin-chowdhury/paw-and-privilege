...
export function BookingForm({ itemLabel }: { itemLabel: string }) {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center gap-3 rounded-2xl border border-emerald/20 bg-emerald/5 p-8 text-center">
        ...
        <p className="font-display text-lg text-ink">Booking request sent</p>
        <p className="text-sm text-ink/60">
          We&rsquo;ve notified the provider about your {itemLabel} request. They&rsquo;ll
          confirm shortly.
        </p>
      </div>
    );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div>
        <Label htmlFor="petName">Pet&rsquo;s name</Label>
        <Input id="petName" className="mt-1.5" {...register("petName")} />
        {errors.petName && (
          <p className="mt-1 text-xs text-destructive">{errors.petName.message}</p>
        )}
      </div>
      <div>
      ...
      <Button type="submit" disabled={isSubmitting} className="w-full rounded-full" size="lg">
        {isSubmitting ? "Sending..." : `Request ${itemLabel}`}
      </Button>
    </form>
  );
}
